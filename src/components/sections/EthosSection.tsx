...
export default function EthosSection() {
  ...
  // offset meaning is per Motion docs: "start end" etc. [web:136]
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end start"],
  });
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 lg:gap-24 items-start">
          ...
          <div className="relative">
            ...
            >
              ...
              <h2 className="text-4xl sm:text-5xl md:text-7xl font-display text-white leading-none">
                MISSION
              </h2>
            </motion.div>

            <p className="max-w-2xl text-xl sm:text-2xl md:text-4xl lg:text-5xl font-display leading-[1.1] flex flex-wrap text-left">
              {words.map((word, i) => {
                const start = i / words.length;
                const end = start + 1 / words.length;
